# verification/verify_cms_stiffness_scan.py
import numpy as np
import sys
import os
import copy
...
if __name__ == '__main__':
    print("--- CMS Stiffness Scan Across Anchor Modes (Validation Step 3) ---")
    
    base_params = load_config('config/config.json')
    q0 = np.zeros(6)
    
    # Define stiffness values and test input
    cms_stiffness_values = [0.05, 0.1, 0.2, 0.4, 0.8]
    ...
    for mode in modes_to_test:
        ...
        for cms_stiff in cms_stiffness_values:
            print(f"--- Testing CMS Stiffness = {cms_stiff:.2f} ---")
