...
def solve_ik(target_pose, params, kappas_guess=None, delta_l_guess=None, use_pso=False, n_particles=10, n_iters=15):
    ...
    if use_pso:
        ...
        def pso_objective_func(delta_l_particles):
            num_particles = delta_l_particles.shape[0]
            costs = np.zeros(num_particles)
            for i in range(num_particles):
                dl_i = delta_l_particles[i]
                # Use default (high-precision) solver options for PSO to ensure accurate cost evaluation
                solve_result = solve_static_equilibrium(kappas_guess, dl_i, params)
                kappas_eq = solve_result["kappas_solution"]
                if kappas_eq is None:
                    costs[i] = 1e6
                    continue
                T_actual, _ = forward_kinematics(kappas_eq, params)
                error_vec = pose_error(T_actual, target_pose)
                costs[i] = np.linalg.norm(error_vec[:3]) * 1000 + np.linalg.norm(error_vec[3:])
            return costs
